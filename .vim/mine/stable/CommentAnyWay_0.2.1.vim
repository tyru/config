...
let s:cs_filetype_table = {
    ...
    \ 'multiline' : {
        ...
        \ },
        \ 'if': {
            \ 'actionscript.c.cpp.cs.d.java.javascript.objc.perl.ruby' : ['if( %c% ) {', '}'],
        \ },
        \ 'while' : {
            \ 'actionscript.c.cpp.cs.d.java.javascript.objc.perl.ruby' : ['while( %c% ) {', '}'],
        \ },
        \ 'for': {
            \ 'actionscript.c.cpp.cs.d.java.javascript.objc.perl.ruby' : ['for( %c%; ; ) {', '}'],
        \ },
        \ 'switch': {
            \ 'actionscript.c.cpp.cs.d.java.javascript.objc.perl.ruby' : ['switch( %c% ) {', '}'],
        \ },
    \ },
\ }
...
func! s:CommentAnyWay.MultiLine.BuildString()
    ...
    for order in s:GetVar( 'cs_multiline_priority' )
        ...
        if has_key( cmts_def, type ) && ! empty( s:GetFileTypeDef( cmts_def[type], [] ) )
            ...
            return s:GetFileTypeDef( cmts_def[type] )
        endif
    endfor
endfunc
" }}}3
