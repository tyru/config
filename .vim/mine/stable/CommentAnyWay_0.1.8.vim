...
"         but parser of |comments| might sometimes get wrong definition.
"
...
func! s:ExecuteNormal( exe_str )
    execute 'nnoremap <Plug>Do '. a:exe_str
    execute "normal \<Plug>Do"
endfunc
" }}}2
" }}}1
"-----------------------------------------------------------------
...
func! s:CommentAnyWay.OneLine.JumpComment() dict
    ...
    call feedkeys( "\<Plug>insert-comment-i", 'm' )    " call s:InsertCommentFromMap( 'i' )
    call feedkeys( "\<Plug>restore-options", 'm' )    " call s:RestoreSetting()
endfunc
" }}}3
...
func! s:CommentAnyWay.MultiLine.Comment() dict
    ...
    " find the line matching '%cursor%' and get its line num.
    let cursor_pos = []
    let i = 0
    while i < len( self.cmt_lis )
        if self.cmt_lis[i] =~# '%cursor%'
            let ind_lnum = self.pos ==# 'o' ? line( '.' ) + 1 : line( '.' )
            let space_len = strlen( self.GetMinIndentSpace( ind_lnum, ind_lnum ) )
            let lnum = line( '.' ) + i + 1
            let col  = space_len + stridx( self.cmt_lis[i], '%cursor%' ) + 1
            let cursor_pos = [0, lnum, col, 0]
        endif
        let i = i + 1
    endwhile
