...
"   0.2.3: Fix bug that GetIndent() get wrong indent num again and again.
" }}}2
"
"
"
...
"
"     cs_multicomment_visual_insert (default:1)
...
"     * convert g:cs_*** to g:ca_***
"   XXX:
"     * func() is really called?
"
"   FIXME:
"     * indent too deeply when intend to indent the line which includes braces.
"     * if exists g:cs_filetype_table, can't load my definitions.
"     
"
"
"==================================================
...
" cs_multicomment_visual_insert
if ! exists( 'g:cs_multicomment_visual_insert' )
    let g:cs_multicomment_visual_insert = 1
endif
...
func! s:CommentAnyWay.Base.GetIndent( lnum ) dict
    ...
    if ! empty( filter( ftypes, 'v:val ==# "c" || v:val ==# "cpp"' ) )
        let indent = cindent( '.' )
    " elseif &lisp && ! empty( filter( ftypes, 'v:val ==# "lisp" || v:val ==# "scheme"' ) )
    "     let indent = lispindent( '.' )
    else
        let indent = indent( '.' )
    endif
...
func! s:CommentAnyWay.MultiLine.Run() dict
    ...
    if ! self.has_range
        ...
        if s:GetVar( 'cs_multicomment_visual_insert' )
