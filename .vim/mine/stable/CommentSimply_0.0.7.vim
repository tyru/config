...
let s:CSObject = {
    ...
    \ 'MultiCommentInfo': { 'replace_pat': '%\([^%]\{-1,}\)%', 'option': { 'cursor': '' } },
    \ 'FileType': {
...
" TODO
" s:CSObject.Function.MultiComment( first, last ) {{{2
func! s:CSObject.Function.MultiComment( first, last )
    ...
    if ! s:CSObject.Function.ExistsMultiLineComment()
    ...
    if a:first ==# a:last
        ...
        if cmt_str !=# ''
            call s:CSObject.Function.InsertString( cmt_str, g:cs_multiline_insert_pos )
        endif
    else
...
" s:CSObject.Function.ExistsMultiLineComment() {{{2
func! s:CSObject.Function.ExistsMultiLineComment()
    let option = s:CSObject.MultiCommentInfo.option
    return has_key( option, 's' ) && has_key( option, 'm' ) && has_key( option, 'e' )
endfunc
" }}}2

" s:CSObject.Function.InsertString( str, pos ) {{{2
func! s:CSObject.Function.InsertString( str, pos )
    let keep_empty = 1

    " build inserted string
    let indent   = s:CSObject.Function.GetIndent( '.' )
...
" s:CSObject.Function.GetIndent( lnum ) {{{2
func! s:CSObject.Function.GetIndent( lnum )
    let cur_line = line('.')
...
func! s:CSObject.Function.LoadVimCommentsOption()
    ...
    let s:CSObject.MultiCommentInfo.option = { 'cursor': '' }

    for i in split( &comments, ',' )
        let [key; vals] = split( i, ':', keep_empty )
        if key == ''
            let no_key_values += filter( copy( vals ), "v:val != ''" )
            continue
        endif
        ...
        if key !~# '0'
            ...
            let s:CSObject.MultiCommentInfo.option[matched] = vals[0]
        endif
    endfor
endfunc
" }}}2
...
func! <SID>MultiComment() range
    call s:CSObject.Function.MultiComment( a:firstline, a:lastline )
endfunc
" }}}2
