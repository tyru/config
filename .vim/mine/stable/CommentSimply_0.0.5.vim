...
let s:CSObject = {
    ...
    \ 'MultiComment': {},
    \ 'FileType': { 'OneLineComment': {}, 'MultiLineComment': {} }
\ }
let s:prev_filetype = ''
let s:mapping_table = {
...
func! s:Init()
    ...
    for [ftype, cmt_str] in g:cs_filetype_table.oneline
        let s:CSObject.FileType.OneLineComment[ftype] = cmt_str
    endfor
...
func! s:LoadWhenBufEnter()
    ...
    if s:prev_filetype != '' && &ft == s:prev_filetype | return | endif

    " set oneline comment string
    if exists( 's:CSObject.FileType.OneLineComment.'. &ft )
        let b:cs_oneline_comment = s:CSObject.FileType.OneLineComment[&ft]
    else
