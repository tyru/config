...
" <SID>RunWithPos( pos ) {{{2
func! <SID>RunWithPos( pos )
...
func! s:Init()
    ...
    for mapkey in keys( g:cs_mapping_table )
        ...
        if ! s:HasAllKeys( map, ['pass', 'silent', 'mode'] )
...
let s:CommentAnyWay = { 'Base' : {}, 'OneLine' : {}, 'MultiLine' : {} }

" s:CommentAnyWay.Base.Init() dict {{{3
"   initialize member variables.
func! s:CommentAnyWay.Base.Init() dict
    let self.pos        = ''
    let self.lnum       = 0
    let self.range      = []
    let self.head_space = ''
...
func! s:CommentAnyWay.Base.GetIndent( lnum_or_expr ) dict
    ...
    "       'O' in Normal mode.
    let self.settings = { 'formatoptions' : &formatoptions, 't_vb' : &t_vb }
...
let s:CommentAnyWay.OneLine['mappings'] = {
    ...
    \ 'v' : { 'funcname' : 'VariuosComment' , 'slurp' : 1 } , 
    \ 'o' : { 'funcname' : 'JumpComment' }, 
    \ 'O' : { 'funcname' : 'JumpComment' }, 
\ }

func! s:CommentAnyWay.OneLine.Init() dict
    call s:CommentAnyWay.Base.Init()
...
func! s:CommentAnyWay.OneLine.Run() dict
    ...
    if self.pos ==# 'i' && ( align_forward || i_read_indent )
...
func! s:CommentAnyWay.OneLine.Comment() dict
    ...
    let is_i_align_cmt = 
                \ self.pos ==# 'i' &&
                \ ( align_forward || ( ! self.HasRange() && i_read_indent ) )
...
func! s:CommentAnyWay.OneLine.JumpComment() dict
    ...
    let self.settings = { 'formatoptions' : &formatoptions }
    setl formatoptions+=o
...
" TODO
" s:CommentAnyWay.OneLine.VariuosComment() dict {{{3
func! s:CommentAnyWay.OneLine.VariuosComment() dict
    ...
    let b:cs_oneline_comment = input( 'set definition:' )
    call self.LoadDefinitions()
    echon 'comment type:'
    call self.RunWithPos( nr2char( getchar() ) )
...
func! <SID>RestoreSetting()
    for type in ['OneLine', 'MultiLine']
        if ! has_key( s:CommentAnyWay[type], 'settings' )
            continue
        endif
        let settings = s:CommentAnyWay[type].settings
        if ! empty( settings )
...
func! s:CommentAnyWay.MultiLine.Comment() dict
    ...
    if ! self.HasRange()
        ...
        call self.InsertString( s:FileType.MultiLineString.range_buffer, 'here' )
    endif
endfunc
" }}}3
