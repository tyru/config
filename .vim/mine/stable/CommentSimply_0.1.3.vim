...
"   0.1.2: Fix some bugs and implement |gco| and |gcO| and 
...
"
"     cs_map_c (default:'t')
"         if this is default value, |gcc| is same as |gct|.
"         allowed 'I', 'i', 'a', 't', 'o', 'O'.
"
...
"         this is used when |gct| and |gct| would comment(not uncomment) the line.
"         if this is default, |gct| would act like |gci| and |gcu| each line.
"
...
"         false: won't insert.
"
"     cs_zero_insert_if_blank (default:1)
"         when |gcI|, if normal mode, and current line is blank line,
"         insert after comment.
"
"     cs_tilda_insert_if_blank (default:1)
"         when |gci|, if normal mode, and current line is blank line,
"         insert after comment.
"
"     cs_dollar_insert (default:1)
"         insert after comment.
"
"     cs_tilda_read_indent (default:1)
...
"
"     TODO: g:cs_filetype_table.wrapline
...
"       use this when history, uncomment(no need select region),
...
"       ]
"
"       I wonder if vim script has unnamed hash or array...
"       (no need that 'time' is out of hash)
"
"
"
"
...
" TODO
" s:CheckVar( var, dict ) {{{2
func! s:CheckVar( var, dict )
endfunc
" }}}2

" }}}1
"-----------------------------------------------------------------
" TODO: check type. ( s:CheckVar() )
" GLOBAL VARIABLES {{{1
...
func! s:LoadOneLineComments()
    ...
    let s:SearchString.UnComment['a']
                ...
                \     '' ]
    " TODO: use s:FileType.WrapString.setting
...
" FIXME: ignore '0' flag (if filetype is 'vim'?)
" s:LoadVimComments() {{{2
...
func! s:CommentOneLine( lnum, pos, space, is_normal )
    ...
    let tilda_read_indent = s:GetVar( 'cs_tilda_read_indent' )
    let is_i_align_cmt    = 
                \ a:pos ==# 'i' &&
                \ ( align_forward || ( a:is_normal && tilda_read_indent ) )
...
func! s:GetIndent( lnum_or_expr )
    let cur_line = line( '.' )    " for returning to original line.
    ...
    if line =~# '^\s*$'
        ...
        execute 'normal O'. substitute( line, '^\s*\(.*\)$', '\1', '' )
    endif
    let indent = indent( '.' )
    if &modified
        delete _
...
func! s:EnterInsertMode( pos, prev_line )
    ...
    if pos ==# 'I'
        try
            let zero_insert_if_blank = s:GetVar( 'cs_zero_insert_if_blank' )
            if prev_line =~# '^\s*$' && zero_insert_if_blank
                call feedkeys( 'A', 'n' )
            endif
        catch /^variable error$/
    ...
    elseif pos ==# 'i'
        try
            let tilda_insert_if_blank = s:GetVar( 'cs_tilda_insert_if_blank' )
            if prev_line =~# '^\s*$' && tilda_insert_if_blank
                call feedkeys( 'A', 'n' )
            endif
        catch /^variable error$/
    ...
    elseif pos ==# 'a'
        try
            let dollar_insert = s:GetVar( 'cs_dollar_insert' )
            if dollar_insert
                call feedkeys( 'A', 'n' )
            endif
        catch /^variable error$/
...
func! <SID>Comment( ... ) range
    ...
    if pos ==# 'i'
        if align_forward || tilda_read_indent
...
" FIXME: strange moving in visual mode and selected multi-line...
...
func! <SID>JumpComment( vect )
    ...
    try
        ...
        " insert empty line.
        call s:InsertString( '', a:vect )
        let lnum = line( '.' )

        " get minimal indent space.
        if pos ==# 'i' && s:GetVar( 'cs_align_forward' )
            let align_space = s:GetMinIndentSpace( lnum, lnum )
        else
            let align_space = ''
        endif
        ...
        " JumpComment() is called in always normal mode.(so arg 4 is 1)
        let replaced = s:CommentOneLine( lnum, pos, align_space, 1 )
        if line != replaced | call setline( lnum, replaced ) | endif
        call s:EnterInsertMode( pos, line )
