...
"       XXX: really work b:*** ?
"
"       and comment string will be replaced with common pattern.
"       '%o%': cs_oneline_comment
"       '%ms%': cs_multiline_template['s']
"       '%mm%': cs_multiline_template['m']
"       '%me%': cs_multiline_template['e']
"       and cs_oneline_comment, cs_multiline_insert_pos['s', 'm', 'e'] are not
...
"
"     cs_map_c (default:'i')    XXX: really work all maps?
"         if this is the default, |gcc| is same as |gci|.
"         allowed 'I', 'i', 'a', 'w', 'm', 't', 'u', 'o', 'O'.
"
...
"         when |gct|, if normal mode, enter insert mode when toggle.
"         this affects cs_I_enter_i_if_blank, cs_i_enter_i_if_blank,
"         cs_a_enter_i.
"           0: no effect.
...
"         and if cs_multiline_read_indent is true, 'm' is ignored.
"
"     cs_multiline_read_indent (default:0)
...
"       not but in this script.(I don't know help-filetype...)
...
"     * I want to insert '# ' when |gci|,
"       and I want to insert '### ' when |gca|.(add global variables)
"     * when |gcm|, insert current indent.
"     * MultiLine.UnComment()
"
"
"
...
func! s:LoadWhenBufEnter()
    ...
    let multicomment         = s:CommentAnyWay.MultiLine
    let multicomment.cmt_lis = multicomment.BuildComments()
endfunc
" }}}2
...
func! s:CommentAnyWay.Base.EnterInsertMode( prev_line ) dict
    ...
    elseif self.pos ==# 'w' && is_blankline
        if s:GetVar( 'cs_wrap_enter_i' )
            let exec = '^'
            let len = strlen( self.GetCmtDef()[2] )    " wrap_forward
...
func! s:InsertCommentFromMap( flag )
    let [fcomment, bcomment, wrap_forward, wrap_back] = s:CommentAnyWay.OneLine.GetCmtDef()
    let str = ''
...
" s:CommentAnyWay.OneLine.GetCmtDef() {{{3
func! s:CommentAnyWay.OneLine.GetCmtDef()
    ...
    " %o% -> oneline_comment, TODO: %ms% -> '/*', etc...
    let wrap_forward = substitute( wrap_forward, '%o%', oneline_comment, 'g' )
    let wrap_back    = substitute( wrap_back, '%o%', oneline_comment, 'g' )
...
func! s:CommentAnyWay.OneLine.LoadDefinitions()
    let oneline_comment = s:GetVar( 'cs_oneline_comment' )
    let [fcomment, bcomment, wrap_forward, wrap_back] = self.GetCmtDef()
...
let s:CommentAnyWay.MultiLine['cmt_lis'] = []    " init in LoadWhenBufEnter().
...
func! s:CommentAnyWay.MultiLine.Comment() dict
    ...
    if empty( self.cmt_lis )
        call s:Warn( 'E003', 'not found cmt_lis. not initialized?' )
        return
    endif
    " find the line matching '%c%' and get its line num.
    let cursor_lnum = 0
    let i = 0
    for i in range( len( self.cmt_lis ) )
        if self.cmt_lis[i] =~# '%c%'
            let cursor_lnum = line( '.' ) + i + 1
            break
        endif
    endfor

    let cmt_str              = join( self.cmt_lis, "\n" )
    let multiline_insert_pos = s:GetVar( 'cs_multiline_insert_pos' )
    let multiline_insert     = s:GetVar( 'cs_multiline_insert' )

    if multiline_insert_pos ==# 'O'
        let cursor_lnum = cursor_lnum - 1
    endif
...
func! s:CommentAnyWay.MultiLine.BuildComments()
    ...
    if g:cs_verbose    " debug
        if empty( result_lis )
            ...
            return result_lis
        elseif len( s:Uniq( values( debug ) ) ) != 1
